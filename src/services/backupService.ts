...
export async function listBackups(): Promise<Backup[]> {
  return invokeCommand<Backup[]>("list_backups");
}
...
export async function restoreBackup(backupId: string): Promise<void> {
  return invokeCommand<void>("restore_backup", { backupId });
}
...
export async function deleteBackup(backupId: string): Promise<void> {
  return invokeCommand<void>("delete_backup", { backupId });
}
...
export async function createBackup(label?: string): Promise<Backup> {
  return invokeCommand<Backup>("create_backup", { label: label ?? null });
}
